    ...
    def forward(
        ...
        **kwargs: Any,
        ...
        if self.pt or self.nn_module:
            y, feats = self.model(im, augment=augment, visualize=visualize, embed=embed, **kwargs)
        ...
        # TensorRT
            ...
            feats = [self.bindings[x].data for x in self.output_names if x != 'output0']
        ...
        if isinstance(y, (list, tuple)):
            ...
            if len(y) == 1 :
                return self.from_numpy(y[0]), feats
            else:
                return [self.from_numpy(x) for x in y], feats
            # return self.from_numpy(y[0]) if len(y) == 1 else [self.from_numpy(x) for x in y]
        else:
            return self.from_numpy(y), feats
