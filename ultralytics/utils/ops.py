...
def non_max_suppression(
    ...
    if isinstance(prediction, (list, tuple)):  # YOLOv8 model in validation model, output = (inference_out, loss_out)
        prediction = prediction[0]  # select only inference output # (inference_out, loss_out)
    ...
    t = time.time()
    output = [torch.zeros((0, 6 + extra), device=prediction.device)] * bs # 初始化输出容器（每个图像一个张量）xyxy + conf + cls
    ...
    for xi, (x, xk) in enumerate(zip(prediction, xinds)):  # image index, (preds, preds indices)
        ...
        if not n:  # no boxes
            continue
        if n > max_nms:  # excess boxes 框数超过限制
            filt = x[:, 4].argsort(descending=True)[:max_nms]  # sort by confidence and remove excess boxes
            x, xk = x[filt], xk[filt] # 按置信度降序排序并截断
